...
def decide_zero(snapshot: EnergySnapshot, current_level: int, settings: dict[str, object]) -> PriDecision:
    """Zéro injection piloté par le réseau, sans cible de consommation maison.

    La réinjection est l'erreur principale à supprimer. Une seule marche RRCR de
    10 % est modifiée par décision. Le prélèvement sert uniquement à autoriser
    une remontée prudente après réduction.
    """
    ...
    # Diagnostic uniquement : la consommation maison n'entre plus dans la décision.
    ...
    # Priorité absolue : tuer la réinjection. Même si import/export sont présents
    # simultanément pendant une transition de compteur, l'export est traité d'abord.
    if snapshot.export_w > export_trigger and current_level > 0:
        ...
        return PriDecision(
            ...
            f"Réinjection {snapshot.export_w:.0f} W > {export_trigger:.0f} W : descente d'une marche pour supprimer l'export.",
            current_score, target_score, house_target,
        )

    # Une remontée n'est permise que lorsqu'un prélèvement réel dépasse la zone
    # acceptable. Le rollback réseau du coordinateur protège ensuite contre une
    # remontée qui recréerait trop de réinjection.
    ...
    if snapshot.export_w <= export_opt and snapshot.import_w <= import_opt:
        reason = "Réseau proche de zéro : maintien du niveau PRI."
    else:
        reason = "Réseau dans l'hystérésis PRI : maintien pour éviter les oscillations."
